 ...
 * 🛡️ SECURITY NOTE: Frontend API keys are vulnerable to exposure.
 * For production, use USE_BACKEND_PROXY = true and move keys to a secure backend.
 */

const GEMINI_API_KEY = process.env.EXPO_PUBLIC__GEMINI_API_KEY;
const GEMINI_API_URL = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent';

// 🛡️ SECURITY TOGGLE: Set to true when backend proxy is ready (e.g., Firebase Cloud Functions)
...
const callGeminiAPI = async (prompt) => {
    ...
    if (!GEMINI_API_KEY || GEMINI_API_KEY === 'your_gemini_api_key_here') {
        console.warn('🛡️ [Security] AI Service: API Key is missing or default. Falling back to local logic.');
...
const buildUserContext = (userData) => {
    ...
    const upcomingExamsCount = exams?.filter(e => !e.completed && new Date(e.date) > new Date()).length || 0;
...
${subjectsByDifficulty.slice(0, 5).map(s => `- ${s.name} (Dificultad: ${s.difficulty}/5)`).join('\n')}
...
${exams?.slice(0, 3).map(e => `- ${e.name} (${subjects?.find(s => s.id === e.subjectId)?.name}) - ${new Date(e.date).toLocaleDateString('es-ES')}`).join('\n')}
    `.trim();
};
...
export const getStudyPlanSuggestion = async (exams, subjects, sessions) => {
    const context = `
Exámenes próximos:
${exams?.slice(0, 5).map(e => `- ID: ${e.id}, Nombre: ${e.name} (${subjects?.find(s => s.id === e.subjectId)?.name}), Fecha: ${new Date(e.date).toLocaleDateString('es-ES')}, Prioridad: ${e.priority}/10`).join('\n')}
...
${sessions.slice(0, 20).map(s => `- ${s.duration} min, ${new Date(s.date).toLocaleDateString('es-ES')} ${new Date(s.date).getHours()}:00`).join('\n')}
    `.trim();
...
export const identifyWeakSubjects = (sessions, subjects, exams) => {
    ...
    const weakSubjects = Object.values(subjectStats).filter(stat => {
        ...
        const notStudiedRecently = !stat.lastStudied ||
...
const getFallbackRecommendations = (userData) => {
    ...
    const upcomingExams = exams?.filter(e => !e.completed && new Date(e.date) > new Date()).slice(0, 3) || [];

    return {
        mainRecommendation: streak?.currentStreak > 0
        ...
        alerts: weakSubjects.length > 0
            ? [`⚠️ ${weakSubjects[0].name} necesita más atención`]
            : []
    };
};
