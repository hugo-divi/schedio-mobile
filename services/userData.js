import { doc, setDoc, getDoc, updateDoc, collection, addDoc, getDocs, query, where, deleteDoc } from 'firebase/firestore';
...
export const saveOnboardingData = async (userId, data) => {
    console.log('saveOnboardingData called with:', { userId, subjectCount: data.subjects?.length, data });
    try {
        // Update user profile
        ...
        if (data.exams && data.exams.length > 0) {
            const examsPromises = data.exams.map(exam =>
                addDoc(collection(db, 'exams'), {
                    ...
                    date: exam.date,
                    priority: calculatePriority(exam.date, exam.difficulty),
                    manualPriority: null,
                    completed: false,
                    createdAt: new Date(),
                })
...
/**
 * Helper: Calculate priority based on days until and difficulty
 */
const calculatePriority = (examDate, difficulty) => {
    const now = new Date();
    const exam = new Date(examDate);
    const daysUntil = Math.ceil((exam - now) / (1000 * 60 * 60 * 24));

    if (daysUntil <= 0) return 10; // Exam is today or past
    return Math.min(10, (1 / daysUntil) * difficulty * 10);
};
