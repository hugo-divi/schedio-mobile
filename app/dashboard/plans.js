import { View, Text, ScrollView, TouchableOpacity, Alert, ActivityIndicator, Dimensions, LayoutAnimation, Platform, UIManager, StyleSheet, Image } from 'react-native';
import { useLocalSearchParams, useRouter } from 'expo-router';
import { Brain, Play, Check, Circle, BookOpen, FolderOpen, CloudUpload, Crown, MoreVertical, AlertCircle, Plus } from 'lucide-react-native';
...
export default function PlansScreen() {
    ...
    const {
        ...
        postponeMicroTask, deleteMicroTask, addManualTask,
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100, paddingHorizontal: 24 }}>
                ...
                {activeTab === 'plans' ? (
                    <View>
                        ...
                        >
                            ...
                            >
                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                                    ...
                                    <View>
                                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                                            <Text style={{ color: '#FFFFFF', fontSize: 16, fontWeight: '800' }}>Planificar con IA</Text>
                                            <Crown size={14} color="#FFCC00" />
                                        </View>
                                        <Text style={{ color: '#FFCC00', fontSize: 12, marginTop: 2, fontWeight: '600' }}>
                                            Schedio Prime
                                        </Text>
                                    </View>
                                </View>
                                {isGenerating ? (
                                    ...
                                    <View style={{ backgroundColor: '#FFCC00', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20 }}>
                                        <Text style={{ color: '#000000', fontWeight: '800', fontSize: 12 }}>GENERAR</Text>
                                    </View>
                                )}
                            </LinearGradient>
                        </TouchableOpacity>
                        ...
                        <Text style={{ color: '#8E8E93', fontSize: 13, marginBottom: 16, paddingHorizontal: 4 }}>
                            Planes automáticos (gratis):
                        </Text>

                        {/* ── Progress Tracker ── */}
                        {todayTasks > 0 && (
                            <View style={{ backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: 16, padding: 16, marginBottom: 24, flexDirection: 'row', alignItems: 'center' }}>
                                <View style={{ width: 48, height: 48, borderRadius: 24, backgroundColor: 'rgba(74, 144, 226, 0.1)', justifyContent: 'center', alignItems: 'center' }}>
                                    <Text style={{ color: '#4A90E2', fontWeight: '800', fontSize: 14 }}>{Math.round(progressPerc)}%</Text>
                                </View>
                                <View style={{ flex: 1, marginLeft: 16 }}>
                                    <Text style={{ color: '#FFF', fontSize: 16, fontWeight: '700' }}>Progreso de Hoy</Text>
                                    <Text style={{ color: '#8E8E93', fontSize: 13, marginTop: 2 }}>{completedToday} de {todayTasks} tareas completadas</Text>
                                </View>
                            </View>
                        )}
                                                    ...
                                                    <View key={item.id} style={{ marginBottom: 12 }}>
                                                        ...
                                                        >
                                                            <GlassCard style={{
                                                                ...
                                                                backgroundColor: item.isPanicMode ? 'rgba(255, 59, 48, 0.05)' : 'rgba(28, 28, 30, 0.95)'
                                                            }}>
                                                                <View
                                                                    style={{
                                                                        position: 'absolute',
                                                                ...
                                                                <View className="flex-1 ml-3">
                                                                    <View className="flex-row items-center mb-1">
                                                                        {item.isPanicMode && <AlertCircle size={10} color="#FF3B30" style={{ marginRight: 4 }} />}
                                                                        <Text style={{ color: item.isPanicMode ? '#FF3B30' : '#D1D5DB', fontSize: 10, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 0.5 }}>
                                                                            {item.phase || 'REPASO'}
                                                                        </Text>
                                                                        <Text style={{ color: item.subjectColor || '#FFF', fontSize: 10, fontWeight: '800', textTransform: 'uppercase', marginLeft: 8 }}>
                                                                    ...
                                                                    <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4 }}>
                                                                        ...
                                                                        {item.isOptional && (
                                                                            <View style={{ backgroundColor: 'rgba(255,255,255,0.1)', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 8, marginLeft: 8 }}>
                                                                                <Text style={{ color: '#A1A1AA', fontSize: 10, fontWeight: '700' }}>Opcional hoy</Text>
                                                                            </View>
                                                                        )}
                                                                    </View>
                                                                </View>
                                                                ...
                                                                <View className="flex-row items-center gap-3 ml-2">
                                                                    <TouchableOpacity
                                                                        onPress={() => {
                                                                            if (Platform.OS === 'web') {
                                                                                if (window.confirm('¿Mover esta tarea para mañana?')) postponeMicroTask(user.uid, item.id);
                                                                            } else {
                                                                                Alert.alert(
                                                                                    "Opciones de Tarea",
                                                                                    "¿Qué deseas hacer con esta tarea?",
                                                                                    [
                                                                                        { text: "Posponer a mañana", onPress: () => postponeMicroTask(user.uid, item.id) },
                                                                                        { text: "Eliminar", onPress: () => deleteMicroTask(user.uid, item.id), style: "destructive" },
                                                                    ...
                                                                    <TouchableOpacity onPress={handleCheck}>
                                                                        <View
                                                                            style={{
                                                                                ...
                                                                                alignItems: 'center',
                                                                                justifyContent: 'center',
                                                                                backgroundColor: item.completed ? (isActive ? '#4A90E2' : '#555') : 'transparent'
                                                                            }}
                                                                        >
                            ...
                            onPress={() => {
                                ...
                                if (Platform.OS === 'web') {
                                    ...
                                    Alert.prompt("Nueva Tarea", "Escribe el título:", [
                                        { text: "Cancelar", style: "cancel" },
                                        { text: "Añadir", onPress: (text) => addManualTask(user.uid, { text, duration: 30 }) }
                                    ]);
                                }
                            }}
                        >
