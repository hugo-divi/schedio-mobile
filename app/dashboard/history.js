...
import { ArrowLeft, Clock, Calendar, ChevronRight, Target, Star, TrendingUp, BarChart2 } from 'lucide-react-native';
...
export default function HistoryScreen() {
    ...
    const { sessionHistory, subjects, loadSessionHistory, stats } = useUserStore();
    ...
    const analytics = useMemo(() => {
        ...
        const topSubjects = Object.entries(subDist)
            .map(([id, time]) => ({
                subject: subjects.find(sub => sub.id === id) || { name: 'Otro', color: tokens.colors.textSecondary },
                time
            }))
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: 100 }}>
                ...
                {analytics && (
                    <View className="mb-10">
                        <Text className="text-textTertiary text-[11px] font-black uppercase tracking-[2px] mb-6">Analíticas Semanales</Text>

                        <GlassCard className="p-6 mb-4 rounded-[32px]" intensity={15}>
                            <View className="flex-row justify-between items-end mb-6">
                                <View>
                                    <Text className="text-white font-black text-2xl">{(stats.totalTime / 60).toFixed(1)}h</Text>
                                    <Text className="text-textTertiary text-[10px] font-bold uppercase">Tiempo Total de Estudio</Text>
                                </View>
                                <View className="items-end">
                                    <Text className="text-primary font-black text-2xl">{analytics.goalRate}%</Text>
                                    <Text className="text-textTertiary text-[10px] font-bold uppercase">Metas Cumplidas</Text>
                                </View>
                            </View>
                            ...
                            <View style={{ height: 120, width: '100%', marginTop: 10 }}>
                                ...
                                <View className="flex-row justify-between mt-2">
                                    {analytics.dailyData.map((day, i) => (
                                        <Text key={i} style={{ color: tokens.colors.textTertiary, fontSize: 9, fontWeight: '900', width: 24, textAlign: 'center' }}>
                                            {day.label.slice(0, 1)}
                                        </Text>
                                    ))}
                                </View>
                        ...
                        <View className="flex-row gap-3">
                            {analytics.topSubjects.map((item, i) => (
                                <GlassCard key={i} className="flex-1 p-4 rounded-2xl items-center" intensity={10}>
                                    <View className="w-8 h-8 rounded-lg mb-2 items-center justify-center" style={{ backgroundColor: item.subject.color + '20' }}>
                                        <Clock size={14} color={item.subject.color} />
                                    </View>
                                    <Text className="text-white font-bold text-[10px]" numberOfLines={1}>{item.subject.name}</Text>
                                    <Text className="text-textTertiary text-[9px] font-black">{formatDuration(item.time)}</Text>
                                </GlassCard>
                            ))}
                        </View>
                    </View>
                ...
                {sessionHistory.length > 0 ? (
                                ...
                                <GlassCard className="rounded-[28px] border-white/5 p-5" intensity={10}>
                                    <View className="flex-row justify-between items-center">
                                        <View className="flex-row items-center flex-1">
                                            <View className="w-12 h-12 rounded-2xl mr-4 items-center justify-center" style={{ backgroundColor: (subject?.color || tokens.colors.primary) + '20' }}>
                                                <Calendar size={22} color={subject?.color || tokens.colors.primary} />
                                            </View>
                                            <View className="flex-1">
                                                <View className="flex-row items-center">
                                                    <Text className="text-white font-black text-lg mr-2">{subject?.name || 'Materia'}</Text>
                                                    {totalGoals > 0 && (
                                                        <View className="bg-primary/20 px-2 py-0.5 rounded-full">
                                                            <Text className="text-primary text-[9px] font-black">{completedGoals}/{totalGoals} METAS</Text>
                                                        </View>
                                                    )}
                                                </View>
                                                <Text className="text-textTertiary text-xs font-bold uppercase">{formatDate(session.date)} • {session.duration} min</Text>
                                            </View>
                                        </View>
                                        <View className="flex-row gap-1">
                                            {[1, 2, 3, 4, 5].map(i => (
                                                <Star
                                                    key={i}
                                                    size={8}
                                                    color={i <= (session.focusScore || 5) ? tokens.colors.yellow : tokens.colors.textTertiary + '40'}
                                                    fill={i <= (session.focusScore || 5) ? tokens.colors.yellow : 'transparent'}
                                                />
                                            ))}
                                        </View>
                                    </View>
                    ...
                    <GlassCard className="p-10 items-center justify-center rounded-[32px] border-dashed border-white/10" intensity={5}>
                        <Target size={40} color={tokens.colors.textTertiary} style={{ marginBottom: 16, opacity: 0.3 }} />
                        <Text className="text-textTertiary font-bold text-center">No hay sesiones registradas aún.{"\n"}¡Empieza a estudiar para ver tu progreso!</Text>
                    </GlassCard>
                )}
