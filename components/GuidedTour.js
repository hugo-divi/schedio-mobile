...
import { View, Text, TouchableOpacity, Modal, StyleSheet, Dimensions, Animated, Platform } from 'react-native';
...
const GuidedTour = ({ onComplete, tourRefs = {}, hasPendingExams = false }) => {
    ...
    const baseSteps = [
        {
            ...
            content: "Tu nuevo aliado para dominar tus estudios. Vamos a darte un minitour rápido por tus herramientas.",
            position: { top: height * 0.3 },
            refKey: null
        },
        {
            title: "El Hub de Control 🧠",
            content: "Aquí verás tus próximas metas, tu racha y la sugerencia de nuestra IA para hoy.",
            position: { top: height * 0.65 },
            refKey: 'heroCardRef'
        },
        {
            title: "Próximos Exámenes 🎯",
            content: "Añade exámenes y tareas aquí para que Schedio pueda planificarte.",
            position: { top: height * 0.1 },
            refKey: 'examsSectionRef'
        }
    ];
    ...
    baseSteps.push({
        ...
        content: "Visualiza tu mes completo de un vistazo. ¡Todo bajo control!",
        position: { top: height * 0.1 },
    ...
    baseSteps.push(
        {
            ...
            content: "Inicia sesiones de estudio enfocadas. Aquí ganarás XP y mantendrás viva tu racha estudiando al menos 5 minutos al día.",
        ...
        {
            title: "Tus Planes 🗺️",
            content: "Aquí Schedio crea automáticamente un plan de estudio diario para cada uno de tus exámenes basado en su dificultad y fecha.",
            position: { top: height * 0.15 },
            refKey: null,
    ...
    const performStepLogic = (currentStepConfig) => {
        ...
        if (currentStepConfig.refKey && tourRefs[currentStepConfig.refKey]?.current && tourRefs.scrollViewRef?.current) {
                    ...
                    (x, y, w, h) => {
                        ...
                        setTimeout(() => {
                            if (tourRefs[currentStepConfig.refKey]?.current) {
                                tourRefs[currentStepConfig.refKey].current.measure((fx, fy, width, height, px, py) => {
            ...
            <View style={StyleSheet.absoluteFill}>
                ...
                <Svg height="100%" width="100%" style={StyleSheet.absoluteFill}>
                    ...
                    <SvgRect
                        ...
                        fill={
                            steps[step].route
                                ? (Platform.OS === 'web' ? 'rgba(0,0,0,0.6)' : 'rgba(0,0,0,0.4)')
                                : (Platform.OS === 'web' ? 'rgba(0,0,0,0.85)' : 'rgba(0,0,0,0.75)')
                        }
                        mask="url(#spotlight)"
                    />
