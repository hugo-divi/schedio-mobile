...
import {
    ...
    Platform, TextInput, Alert, ScrollView, KeyboardAvoidingView
} from 'react-native';
import { BlurView } from 'expo-blur';
import { X, CalendarPlus, FileText, Package, Sparkles, Send, Star, ChevronRight, Check, Lock } from 'lucide-react-native';
...
export default function QuickActionsModal({ visible, onClose, onAddExam, onAddFile, completedExams = [], onRefresh }) {
    const router = useRouter();
    const { isPrime, addQuickNote } = useUserStore();
    ...
    const handleSaveGrade = async () => {
        ...
        try {
            ...
            setGradeInput('');
            setSelectedExam(null);
            Alert.alert('✓ Nota guardada', `Has registrado un ${num} (${(weight * 100).toFixed(0)}%) en ${selectedExam.name}.`);
        } catch {
    ...
    const actions = [
        {
            ...
            color: '#4A90E2',
            onPress: () => { handleClose(); onAddExam(); },
        },
        {
            id: 'grade',
            icon: <Star size={22} color="#FF9F0A" />,
            label: 'Nota de Examen',
            desc: examsWithoutGrade.length > 0 ? `${examsWithoutGrade.length} sin calificar` : 'Sin exámenes pendientes',
            color: '#FF9F0A',
    ...
    const renderHeader = (title, hideClose = false) => (
        <View style={styles.header}>
            ...
            {hideClose ? <View style={styles.pill} /> : (
                                ...
                                {actions.map(action => (
                                    ...
                                    >
                                        ...
                                        <View style={styles.actionText}>
                                            <Text style={[styles.actionLabel, action.disabled && { color: tokens.colors.textSecondary }]}>
                                                {action.label}
                                            </Text>
                        ...
                        {view === VIEW_NOTE && (
                            <>
                                ...
                                >
                                    ...
                                    <Text style={styles.primaryBtnText}>{savingNote ? 'Guardando...' : 'Guardar Nota'}</Text>
                                </TouchableOpacity>
                            </>
                        )}
                            ...
                            <>
                                ...
                                {examsWithoutGrade.length === 0 ? (
                                    <View style={styles.emptyState}>
                                        <Star size={36} color={tokens.colors.textSecondary} style={{ marginBottom: 12, opacity: 0.3 }} />
                                        <Text style={styles.emptyText}>Todos los exámenes ya tienen nota registrada.</Text>
                                    </View>
                                ) : (
                                    <ScrollView style={styles.examList} showsVerticalScrollIndicator={false}>
                                        {examsWithoutGrade.map(exam => (
                                            <TouchableOpacity
                                                ...
                                                onPress={() => { setSelectedExam(exam); setView(VIEW_GRADE_INPUT); }}
                        ...
                        {view === VIEW_GRADE_INPUT && selectedExam && (
                            <>
                                ...
                                <View style={{ flexDirection: 'row', gap: 12, width: '100%' }}>
                                    <TouchableOpacity
                                        style={[styles.gradeInputWrapper, { flex: 2 }, gradeError && styles.gradeInputError]}
                                        activeOpacity={1}
                                    ...
                                    >
                                        ...
                                        <TextInput
                                            ...
                                            autoFocus
                                            value={gradeInput}
                                            onChangeText={t => { setGradeInput(t); setGradeError(''); }}
                                            maxLength={4}
                                            selectionColor={tokens.colors.primary}
                                        />
                                    </TouchableOpacity>
                                    ...
                                    >
                                        ...
                                        <TextInput
                                            ...
                                            value={weightInput}
                                            onChangeText={t => { setWeightInput(t); setGradeError(''); }}
                                            maxLength={3}
                                            selectionColor={tokens.colors.primary}
                                        />
                                    </TouchableOpacity>
                                ...
                                >
                                    ...
                                    <Text style={styles.primaryBtnText}>{savingGrade ? 'Guardando...' : 'Guardar Nota'}</Text>
                                </TouchableOpacity>
                            </>
                        )}
