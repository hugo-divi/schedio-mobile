...
import { doc, getDoc, updateDoc, collection, getDocs, query, limit, addDoc, deleteDoc, where } from 'firebase/firestore';
import { getUserSubjects, updateUserProfile, createSubject, updateSubject, deleteSubject } from '../services/userData';
import { createSession } from '../services/sessions';
import { calculateLevel, getRankForLevel, checkNewBadges, BASE_XP_PER_MINUTE } from '../services/gamification';
...
const useUserStore = create((set, get) => ({
        ...
        try {
            ...
            if (userSnap.exists()) {
                ...
                set({
                    ...
                    isPrime: data?.isPrime || false, // Load Prime status
    ...
    canUpload: () => {
        const { isPrime, uploadsHistory, profile } = get();
    ...
    setPrimeStatus: async (userId, isPrime) => {
        set({ isPrime });
        try {
            const userRef = doc(db, "users", userId);
            await updateDoc(userRef, { isPrime });
        } catch (error) {
            console.error("Error setting Prime status:", error);
        }
    },
    ...
    addSession: async (uid, sessionData) => {
        ...
        // 3. Badges
        const unlockedBadges = checkNewBadges(gamification.badges, newStats, { duration: sessionData.duration });
    ...
    updateAverageGrade: async (userId) => {
        try {
            ...
            if (completedExams && completedExams.length > 0) {
                const gradesWithValues = completedExams.filter(e => e.grade !== undefined && e.grade !== null);
    ...
    initDailyMicroplans: async (uid, force = false) => {
        ...
        const shouldGenerate = force || microplans.length === 0 || stats.lastPlanGenerationDate !== today;
    ...
    addManualTask: async (uid, taskData) => {
        ...
        const updatedPlans = [...microplans, newTask]
    ...
    postponeMicroTask: async (uid, taskId) => {
        ...
        const updatedPlans = microplans.map(task =>
            task.id === taskId ? { ...task, date: tomorrow.toISOString() } : task
        ).sort((a, b) => new Date(a.date) - new Date(b.date));
